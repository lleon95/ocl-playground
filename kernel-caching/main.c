...
#define BIN_PATH "./vector-add.bin"
#define CL_PATH "../vector-vector-add-baseline/vector-add.cl"
/*
 * Utility for reading files and setting the corresponding length
 */
...
int main(int argc, char **argv) {
  ...
  if (compile) {
    ...
    long bin_size;
    binary = common_read_file(BIN_PATH, &bin_size);
    source_size = bin_size;
  ...
  if (compile) {
    ...
    binary = malloc(source_size);
    clGetProgramInfo(program, CL_PROGRAM_BINARIES, source_size, &binary, NULL);
    fp = fopen(BIN_PATH, "w");
    fwrite(binary, source_size, 1, fp);
    fclose(fp);
  }
